/*
 * mango/mach/mach_port.h
 *
 * Mach port abstraction for the Mango nanokernel.
 *
 * In real Mach, ports are kernel objects that name a message queue.
 * In Mango, each port maps to a Unix domain socket pair, giving
 * us bidirectional message passing within the host POSIX environment.
 */

#ifndef MANGO_MACH_PORT_H
#define MANGO_MACH_PORT_H
