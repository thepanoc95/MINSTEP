...
- (kern_return_t)terminate:(mango_task_t *)task {
...
- (kern_return_t)insertPort:(mango_task_t *)task :(mach_port_name_t)name :(mach_port_t)port {
...
- (kern_return_t)removePort:(mango_task_t *)task :(mach_port_name_t)name {
...
- (mach_port_t)bootstrapPort:(mango_task_t *)task {
    if (!task) return MACH_PORT_NULL;
    return task->bootstrap_port;
}

- (kern_return_t)threadCreate:(mango_task_t *)task :(mango_thread_t **)out {
...
- (kern_return_t)threadTerminate:(mango_thread_t *)thread {
...
- (kern_return_t)threadSuspend:(mango_thread_t *)thread {
    if (!thread) return KERN_INVALID_ARGUMENT;
    thread->suspended = YES;
    return KERN_SUCCESS;
}

- (kern_return_t)threadResume:(mango_thread_t *)thread {
    if (!thread) return KERN_INVALID_ARGUMENT;
    thread->suspended = NO;
    return KERN_SUCCESS;
}
...
- (kern_return_t)launchInit:(const char *)root initPath:(const char *)init_path_arg {
    ...
    pid_t pid = fork();
    if (pid < 0) {
        klog_sub_err("init", "fork failed: %s\n", strerror(errno));
    ...
    if (pid == 0) {
        ...
        klog_sub_err("init", "exec failed: %s\n", strerror(errno));
