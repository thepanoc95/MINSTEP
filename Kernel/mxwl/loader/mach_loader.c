/*
 * mxwl/loader/mach_loader.c
 *
 * .mach binary loader for the Maxxwell nanokernel.
 */
...
#define MACH_LOADER_TMPDIR "/tmp/mxwl-loader"

kern_return_t mxwl_loader_init(void)
{
...
kern_return_t mxwl_loader_validate(const char *mach_path,
...
kern_return_t mxwl_loader_extract_binary(const char *mach_path,
...
kern_return_t mxwl_loader_exec(mxwl_task_t *task,
                                ...
                                const mach_binary_header_t *header)
{
    ...
    char tmp_path[512];
    kern_return_t kr = mxwl_loader_extract_binary(mach_path, header,
    ...
    kal_env_set("MXWL_TASK_ID", id_str, 1);
...
kern_return_t mxwl_loader_load(const char *mach_path,
                                mxwl_task_t **out_task)
{
    if (!mach_path) return KERN_INVALID_ARGUMENT;

    mach_binary_header_t header;
    kern_return_t kr = mxwl_loader_validate(mach_path, &header);
    if (kr != KERN_SUCCESS) return kr;

    mxwl_task_t *task = NULL;
    kr = mxwl_task_create(&task, mach_path);
    if (kr != KERN_SUCCESS) return kr;

    kr = mxwl_loader_exec(task, mach_path, &header);
    if (kr != KERN_SUCCESS) {
...
kern_return_t mxwl_loader_cleanup(mxwl_task_t *task)
{
