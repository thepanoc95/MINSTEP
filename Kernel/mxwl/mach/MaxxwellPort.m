...
mach_port_object_t mxwl_port_table[MACH_PORT_TABLE_SIZE];
int                mxwl_port_table_count = 0;

@implementation MaxxwellPort

- (id)init {
    self = [super init];
    if (self) {
        memset(mxwl_port_table, 0, sizeof(mxwl_port_table));
        mxwl_port_table_count = 0;
    }
    return self;
}
...
- (id)free {
    for (int i = 0; i < MACH_PORT_TABLE_SIZE; i++) {
        if (mxwl_port_table[i].in_use) {
            int portName = mxwl_port_table[i].name;
...
- (mach_port_object_t *)lookup:(mach_port_t)port {
    ...
    mach_port_object_t *obj = &mxwl_port_table[port - 1];
    if (!obj->in_use) {
        return NULL;
    }
